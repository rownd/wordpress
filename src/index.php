...
if (!class_exists('Rownd_Auth_Class')) {

	class Rownd_Auth_Class
	{
		...
		function __construct()
		{
			...
			add_action('init', array($this, 'signin_handler')); //check for the social logins
		...
		function main_page()
		{
			include('includes/backend/main.php');
		}
		...
		function save_settings()
		{
			if (isset($_POST['rownd_save_settings']) && $_POST['rownd_settings_action'] && wp_verify_nonce($_POST['rownd_settings_action'], 'rownd_nonce_save_settings')) {
				include('includes/backend/save-settings.php');
			} else {
		...
		function authenticate_user() {
			include('includes/frontend/sign-in-handler.php');
		}
...
$rownd_instance = new Rownd_Auth_Class();
